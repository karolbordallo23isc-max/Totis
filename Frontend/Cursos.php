...
// Obtener todos los módulos con su curso
$stmtModulos = $pdo->query("SELECT m.id_modulo, m.nombre, m.descripcion, c.id_curso 
                             FROM modulos m 
                             JOIN curso c ON m.id_curso = c.id_curso 
                             ORDER BY m.id_curso, m.orden");
$modulos = $stmtModulos->fetchAll(PDO::FETCH_ASSOC);

// Calcular progreso real por módulo usando respuesta_abierta
$progreso_modulos = [];
foreach ($modulos as $mod) {
    $id_modulo = $mod["id_modulo"];

    // Total de ejercicios distintos en este módulo
    $stmtTotal = $pdo->prepare("SELECT COUNT(DISTINCT id_ejercicio) FROM ejercicios WHERE id_modulo = ?");
    $stmtTotal->execute([$id_modulo]);
    $total = (int) $stmtTotal->fetchColumn();

    // Cuántos de esos ejercicios ya tienen respuesta de este usuario
    $stmtComp = $pdo->prepare("
        SELECT COUNT(DISTINCT e.id_ejercicio) 
        FROM ejercicios e
        JOIN respuesta_abierta r ON r.id_ejercicio = e.id_ejercicio
        WHERE e.id_modulo = ? AND r.id_usuario = ?
    ");
    $stmtComp->execute([$id_modulo, $id_usuario]);
    $completados = (int) $stmtComp->fetchColumn();

    // Asegurarse que completados nunca supere total
...
<main class="main">
    ...
    <section class="content">
        <h2 style="margin-bottom:5px;">Módulos</h2>
                   ...
                   data-progreso="<?= $pct ?>">
                    ...
                    <div class="footer-card">
                        <span class="lecciones-info">
                            <?= $p["completados"] ?>/<?= $p["total"] ?> ejercicios respondidos
                        </span>
                        <?php if ($done): ?>
