...
class Admin {
    ...
    public static function allUsers(): array {
        $stmt = getDB()->query(
            'SELECT u.id_usuario, u.nombre, u.correo, u.usuario, u.is_admin,
                    COUNT(p.id_progreso) AS ejercicios_completados
             FROM usuario u
    ...
    public static function userProgress(int $userId): array {
        $stmt = getDB()->prepare(
            'SELECT m.nombre AS modulo, c.titulo AS leccion,
                    e.pregunta, p.completado, p.fecha_completado
             FROM progreso p
